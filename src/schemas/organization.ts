...
export const organizationSchema = z.object({
  ...
  ).trim().toLowerCase().min(3, 'El nombre de la organización debe tener al menos 3 caracteres'),
  description: z.string(
    { invalid_type_error: 'Descripción Invalida', required_error: 'La descripción es requerida' }
  ).trim().toLowerCase().min(10, 'La descripción debe tener al menos 10 caracteres'),
